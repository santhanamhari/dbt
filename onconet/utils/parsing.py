...
def parse_args():
    ...
    parser.add_argument('--patience', type=int, default=20, help='number of epochs without improvement on dev before halving learning rate and reloading best model [default: 5]')
    ...
    # Add these new arguments inside parse_args() after the existing device arguments
    # (after line ~338 where --data_parallel is defined):

    # Distributed training (multi-node)
    parser.add_argument('--distributed', action='store_true', default=False, help='Use DistributedDataParallel for multi-node training')
    parser.add_argument('--local_rank', type=int, default=0, help='Local rank for distributed training. Set automatically by torchrun/srun.')
    parser.add_argument('--dist_backend', type=str, default='nccl', help='Distributed backend [default: nccl]')
    parser.add_argument('--dist_url', type=str, default='env://', help='URL for distributed init [default: env://]')
    # visualization
