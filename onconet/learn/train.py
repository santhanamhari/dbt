...
def ddp_all_gather_list(local_list):
    world_size = dist.get_world_size()
    #print(f"[Rank {dist.get_rank()}] world_size: {world_size}, local_list length: {len(local_list)}")
    ...
    for g in gathered:
        merged.extend(g)
    #if dist.get_rank() == 0:
    #    print(f"[Rank {dist.get_rank()}] merged length: {len(merged)}")
    return merged

def is_main_process(args):
    """Returns True if this is rank 0 or non-distributed."""
...
def train_model(train_data, dev_data, model, args):
    ...
    _is_main = is_main_process(args)
    is_distributed = hasattr(args, 'distributed') and args.distributed

    for epoch in range(start_epoch, args.epochs + 1):

        # For DDP: set epoch on DistributedSampler so each epoch shuffles differently
        if is_distributed:
            args.train_sampler.set_epoch(epoch)
            args.dev_sampler.set_epoch(epoch)
        ...
        for mode, data_loader in [('Train', train_data_loader), ('Dev', dev_data_loader)]:
            ...
            if is_distributed: #and key_prefix == 'dev':
                golds_gather = ddp_all_gather_list(golds)
                preds_gather = ddp_all_gather_list(preds)
                probs_gather = ddp_all_gather_list(probs)
                exams_gather = ddp_all_gather_list(exams)
                censor_times_gather = ddp_all_gather_list(censor_times)

                if key_prefix == 'dev':
                    # 1. Build a mapping from exam to [index, values], keeping first occurrence
                    exam_to_indices = OrderedDict()
                    for i, exam in enumerate(exams_gather):
                        if exam not in exam_to_indices:
                            exam_to_indices[exam] = i  # Store index of first occurrence

                    # 2. Get indices to keep (sorted by exam order)
                    unique_indices = list(exam_to_indices.values())

                    # 3. Filter all arrays/lists by unique_indices
                    exams        = [exams_gather[i]        for i in unique_indices]
                    golds        = [golds_gather[i]        for i in unique_indices]
                    preds        = [preds_gather[i]        for i in unique_indices]
                    probs        = [probs_gather[i]        for i in unique_indices]
                    censor_times = [censor_times_gather[i] for i in unique_indices]
                    ...
                    probs = probs_gather
                    exams = exams_gather
                    censor_times = censor_times_gather
    ...
    if _is_main:
        print("Restoring best model from epoch {}".format(epoch_stats['best_epoch'] + 1))
        models, _, _, _, _ = state_keeper.load()

    # everyone waits until rank 0 finished loading
    if is_distributed:
        dist.barrier()
