...
EXCLUDED_ARGS = ['start_time', 'model_path', 'optimizer_state', 'current_epoch', 'lr',
                 'epoch_stats', 'resume', 'patient_to_partition_dict', 'path_to_hidden_dict',
                 'train_sampler', 'dev_sampler']  # exclude DDP samplers from hash
...
class StateKeeper():
    ...
    def _unwrap_model(self, model):
        """Unwrap DDP or DataParallel to get the raw module for saving."""
    ...
    def save(self, models, optimizers, epoch, lr, epoch_stats):
        ...
        for model_name in models:
            # Unwrap DDP/DataParallel before saving
            model = self._unwrap_model(models[model_name])
    ...
    def load(self):
        ...
        # Determine map_location for DDP
        if hasattr(self.args, 'distributed') and self.args.distributed:
            map_location = {'cuda:0': 'cuda:{}'.format(self.args.local_rank)}
        else:
            map_location = None
        ...
        for model_name in model_names:
            ...
            model_path = os.path.join(
                                ...
                                "{}_{}".format(
                                    model_name, MODEL_PATH.format(identifier)))
            try:
                models[model_name] = torch.load(model_path, map_location=map_location)
            except:
                raise Exception(
            ...
            optimizer_path = os.path.join(
                                ...
                                "{}_{}".format(
                                    model_name, OPTIMIZER_PATH.format(identifier)))
            try:
                optimizer_states[model_name] = torch.load(optimizer_path, map_location=map_location)
            except:
                raise Exception(
