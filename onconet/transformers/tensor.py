...
class _ForceNumChan:
    def __init__(self, num_chan: int):
        self.num_chan = int(num_chan)

    def __call__(self, tensor):
        existing_chan = tensor.size(0)
        if existing_chan != self.num_chan:
            return tensor.expand(self.num_chan, *tensor.size()[1:])
        return tensor
...
class Force_Num_Chan_Tensor_2d(Abstract_transformer):
    def __init__(self, args, kwargs):
        super().__init__()
        assert len(kwargs) == 0
        self.transform = _ForceNumChan(args.num_chan)
